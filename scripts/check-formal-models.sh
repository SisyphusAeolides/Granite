...
idris="${IDRIS2:-idris2}"
agda="${AGDA:-agda}"
idris_source="$root/formal/idris2/GraniteBoot.idr"
agda_source="$root/formal/agda/GraniteHandoff.agda"
...
scratch="$(mktemp -d "${TMPDIR:-/tmp}/granite-formal.XXXXXXXX")"
trap 'rm -rf -- "$scratch"' EXIT
cp "$idris_source" "$agda_source" "$scratch/"
(
    cd "$scratch"
    "$idris" --check GraniteBoot.idr
    XDG_DATA_HOME="$scratch/data" XDG_CONFIG_HOME="$scratch/config" \
        "$agda" --no-libraries --safe --without-K GraniteHandoff.agda
)
